...
/**
 * Multi-regions scan function. Invokes all activated decoders by priority.
 * For successful scan, allocates pp_data buffer and pass it to user.
 * User should deallocate *pp_data pointer when no more needed.
 *
 * @param[in]   pp_image                Byte array representing grayscale value of image pixels.
 ...
 * @param[in]   lenX                    X axis size (width) of image.
 * @param[in]   lenY                    Y axis size (length) of image.
 * @param[in]   regionsData             Rectangles data in form of continuous array of left, top, width, height expressed
 *                                      in percentages of image size
 * @param[in]   numberOfRegions         Number of rectangles provided in regionsData array
 * @param[in]   maxThreads              Maximum number of threads to be used in decoding
 * @param[out]  pp_data                 On successful decode, library allocates new byte array where it stores decoded
 *                                      string result. Pointer to string is passed here. User application is responsible
 *                                      for deallocating this buffer after use.
 *
 ...
 * @retval      MWB_RT_BAD_PARAM        Null pointer or out of range parameters
 * @retval      MWB_RT_NOT_SUPPORTED    Unsupported decoder found in execution list - library error
 * @retval      MWB_RT_INVALID_SCANNING_RECTANGLE   Scanning rectangle for all active barcodes must be set to 0,0,100,100
 */
extern int MWB_scanGrayscaleRegions(uint8_t*  pp_image,  int lenX,  int lenY, float* regionsData, int numberOfRegions, int maxThreads, uint8_t **pp_data);


/**
 * Create multiple regions based on number of tiles by X and Y axes
 *
 * @param[in]   tilesX                  Number of tiles by X axis
 * @param[in]   tilesY                  Number of tiles by Y axis
 * @param[in]   overlap                 How much each region should overlap neighbour one, expressed in percentage of image size
 * @param[out]  regionsData             pre-allocated array for regions rectangles, must be of size tilesX * tilesY * 4 at least
 *
 * @retval      >0                      Number of regions created
 * @retval      MWB_RT_BAD_PARAM        Null pointer or out of range parameters
 */
extern int MWB_createRegionsFromTiles(int tilesX, int tilesY, int overlap, float *regionsData);
